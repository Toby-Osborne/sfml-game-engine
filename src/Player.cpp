...
void Player::_handle_player_physics(const sf::Vector2f &movement_input) {
    ...
    // Handle X input
    if ((((velocity.x > 1.0)||(velocity.x < -1.0))&&(sign(velocity.x) != sign(movement_input.x)))||(movement_input.x == 0)) {
        velocity.x*=0.9;
    ...
    // Handle Y input
    if ((((velocity.y > 1.0)||(velocity.y < -1.0))&&(sign(velocity.y) != sign(movement_input.y)))||(movement_input.y == 0)) {
        velocity.y *= 0.9;
