...
bool Entity::will_collide_y(float dy) {
    float y_coord = dy > 0 ? _this_hitbox._y_positive_bound : -_this_hitbox._y_negative_bound;

    if (_is_point_colliding((int) (_this_hitbox._x_positive_bound + _entity_coordinates.x) / TILE_SIZE,
                            (int) (y_coord + _entity_coordinates.y + dy) / TILE_SIZE))
        return true;
...
bool Entity::will_collide_x(float dx) {
    float x_coord = dx > 0 ? _this_hitbox._x_positive_bound : -_this_hitbox._x_negative_bound;

    if (_is_point_colliding((int) (x_coord + _entity_coordinates.x + dx) / TILE_SIZE,
